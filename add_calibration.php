...
$stmt = $pdo->query("SELECT COUNT(*) AS total FROM calibration_report");
...
while ($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)) {
    $calibrators_arr[] = $row2['calibrator'];
}
...
if (isset($_POST['submit'])) {
    ...
    $model            = $_POST['model'];
    $maker            = $_POST['maker'];
    $model_maker      = trim($model) . '/' . trim($maker);
    ...
    $stmt = $pdo->prepare("INSERT INTO calibration_report
        (item_no, description, machine_code, model_maker, model, maker, serial_number,
         location, section, manager, reference, calibration_date, next_calibration,
         cal_frequency, calibrator, cal_type, env_type, present_status, remarks)
        VALUES
        (:item_no, :description, :machine_code, :model_maker, :model, :maker, :serial_number,
         :location, :section, :manager, :reference, :calibration_date, :next_calibration,
         :cal_frequency, :calibrator, :cal_type, :env_type, :present_status, :remarks)");
    ...
    try {
        ...
        log_audit($pdo, 'INSERT', 'calibration_report', $newId,
            "Added: {$description} | Code: {$machine_code} | Status: {$present_status}");
...
<title>Add Calibration — Calibration Management</title>
...
.form-group input.auto-value::placeholder{color:#86efac;}
...
<div class="main-content">
    ...
    <div class="card">
        <div class="card-header">
            ...
            <p>Fill in all required fields to register a new equipment entry.</p>
        </div>
        <div class="card-body">
            ...
            <form method="POST">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label>Item No.</label>
                        <input type="text" name="item_no" value="<?= $next_item_no ?>" class="auto-value" readonly>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="machine_code" class="mono-input"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="model" id="model_input"
                               <?= $isGuest ? 'readonly class="readonly"' : '' ?>
                               oninput="syncModelMaker()">
                    </div>

                    <div class="form-group">
                        <label>Maker</label>
                        <input type="text" name="maker" id="maker_input"
                               <?= $isGuest ? 'readonly class="readonly"' : '' ?>
                               oninput="syncModelMaker()">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="model_maker" id="model_maker_display"
                               class="auto-value" readonly placeholder="Filled from Model + Maker">
                    </div>

                    <div class="form-group">
                        <label>Serial Number <span class="required-star">*</span></label>
                        <input type="text" name="serial_number" class="mono-input"
                               <?= $isGuest ? 'readonly' : 'required' ?>
                               >
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="reference" class="mono-input"
                               <?= $isGuest ? 'readonly class="readonly"' : '' ?>
                               >
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="next_calibration" id="next_calibration"
                               class="auto-value" readonly>
                    </div>

                    <div class="form-group">
                        <label>Calibration Frequency <span class="required-star">*</span></label>
                        <select name="cal_frequency" id="cal_frequency"
                                onchange="updateNextCalibration()"
                        ...
                        <?php if ($isGuest): ?>
                        <input type="hidden" name="cal_frequency" value="">
                        <?php endif; ?>
                    </div>

                    <div class="form-group">
                        <label>Calibrator <span class="required-star">*</span></label>
                        <input type="text" name="calibrator" id="calibrator_input"
                               <?= $isGuest ? 'readonly class="readonly"' : 'required' ?>
                               autocomplete="off">
                    ...
                    <div class="form-group">
                        ...
                        <select name="cal_type" <?= $isGuest ? 'disabled class="readonly"' : '' ?>>
                            <option value="">— Select —</option>
                            <option value="Internal Calibration">Internal Calibration</option>
                            <option value="External Calibration">External Calibration</option>
                        </select>
                        <?php if ($isGuest): ?>
                        <input type="hidden" name="cal_type" value="">
                        <?php endif; ?>
                    </div>

                    <div class="form-group">
                        <label>Environment Type</label>
                        <select name="env_type" <?= $isGuest ? 'disabled class="readonly"' : '' ?>>
                            <option value="">— Select —</option>
                            <option value="Environmental">Environmental</option>
                            <option value="Non-Environmental">Non-Environmental</option>
                        </select>
                        <?php if ($isGuest): ?>
                        <input type="hidden" name="env_type" value="">
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <?php if ($isGuest): ?>
                        <input type="hidden" name="present_status" value="Good">
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="form-actions">
                        <a href="calibration_report.php?page=<?= $returnPage ?>" class="btn btn-muted">
...
function syncModelMaker() {
    ...
    const parts = [model, maker].filter(Boolean);
    document.getElementById('model_maker_display').value = parts.join('/');
}
