...
$cancelUrl     = $fromDashboard ? 'dashboard.php' : 'calibration_report.php?page=' . $returnPage;
...
$display_item_no = 0;
$counter = 0;
$list = $pdo->query("SELECT id FROM calibration_report ORDER BY id ASC");
...
$calibrators_arr = [];
$stmt2 = $pdo->query("SELECT DISTINCT calibrator FROM calibration_report WHERE calibrator IS NOT NULL AND calibrator != '' ORDER BY calibrator ASC");
while ($r2 = $stmt2->fetch(PDO::FETCH_ASSOC)) { $calibrators_arr[] = $r2['calibrator']; }

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($isGuest) { header("Location: " . $cancelUrl); exit(); }

    $stmt = $pdo->prepare("
        UPDATE calibration_report SET
            description=:description, machine_code=:machine_code,
            model_maker=:model_maker, model=:model, maker=:maker,
            serial_number=:serial_number, location=:location,
            section=:section, manager=:manager, reference=:reference,
            calibration_date=:calibration_date, next_calibration=:next_calibration,
            cal_frequency=:cal_frequency, calibrator=:calibrator,
            cal_type=:cal_type, env_type=:env_type, present_status=:present_status,
            remarks=:remarks
        WHERE id=:id
    ");
    try {
        $stmt->execute([
            ...
            ':model_maker'      => trim($_POST['model']).'/'.trim($_POST['maker']),
            ':model'            => $_POST['model'],
            ':maker'            => $_POST['maker'],
        ...
        log_audit($pdo, 'UPDATE', 'calibration_report', $id,
            "Updated: {$_POST['description']} | Code: {$_POST['machine_code']} | Status: {$_POST['present_status']}");
...
function normalizeCalType($val) {
    if ($val === 'Internal') return 'Internal Calibration';
    if ($val === 'External') return 'External Calibration';
    return $val ?? '';
}
$calTypeNormalized = normalizeCalType($row['cal_type'] ?? '');
...
<title>Edit Calibration Record — Calibration Management</title>
...
.form-group textarea{padding:8px 12px;resize:vertical;line-height:1.55;min-height:72px;}
...
/* Auto-computed — blue (item no, model/maker) */
...
/* Auto-computed — green (next calibration) */
    ...
    <div class="card">
        <div class="card-header">
            <div class="card-header-left">
                <h2>Edit Calibration Record</h2>
                <p><?= htmlspecialchars($row['machine_code']) ?> — <?= htmlspecialchars($row['description']) ?></p>
            </div>
        ...
        <div class="card-body">
            ...
            <form method="POST" id="editCalForm">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label>Machine Code <span class="required-star">*</span></label>
                        <input type="text" name="machine_code" class="mono-input" value="<?= htmlspecialchars($row['machine_code']) ?>" <?= $isGuest ? 'readonly' : 'required' ?>>
                    </div>

                    <div class="form-group">
                        <label>Model</label>
                        <input type="text" name="model" id="model_input" value="<?= htmlspecialchars($row['model'] ?? '') ?>" <?= $isGuest ? 'readonly' : '' ?> placeholder="e.g. 500-196" oninput="syncModelMaker()">
                    </div>

                    <div class="form-group">
                        <label>Maker</label>
                        <input type="text" name="maker" id="maker_input" value="<?= htmlspecialchars($row['maker'] ?? '') ?>" <?= $isGuest ? 'readonly' : '' ?> placeholder="e.g. Mitutoyo" oninput="syncModelMaker()">
                    </div>

                    <div class="form-group">
                        <label>Model / Maker <span class="autofill-pill">auto</span></label>
                        <input type="text" name="model_maker" id="model_maker_display" value="<?= htmlspecialchars($row['model_maker'] ?? '') ?>" class="auto-value-blue" readonly>
                    </div>

                    <div class="form-group">
                        <label>Serial Number <span class="required-star">*</span></label>
                        <input type="text" name="serial_number" class="mono-input" value="<?= htmlspecialchars($row['serial_number']) ?>" <?= $isGuest ? 'readonly' : 'required' ?>>
                    </div>
                    ...
                    <div class="form-group">
                        <label>Section</label>
                        <input type="text" name="section" value="<?= htmlspecialchars($row['section'] ?? '') ?>" <?= $isGuest ? 'readonly' : '' ?> placeholder="e.g. QA Section">
                    </div>

                    <div class="form-group">
                        <label>Manager</label>
                        <input type="text" name="manager" value="<?= htmlspecialchars($row['manager'] ?? '') ?>" <?= $isGuest ? 'readonly' : '' ?> placeholder="e.g. Juan Dela Cruz">
                    </div>

                    <div class="form-group">
                        <label>Reference</label>
                        <input type="text" name="reference" class="mono-input" value="<?= htmlspecialchars($row['reference'] ?? '') ?>" <?= $isGuest ? 'readonly' : '' ?> placeholder="e.g. REF-2024-001">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="next_calibration" id="next_calibration" value="<?= $row['next_calibration'] ?>" class="auto-value-green" readonly>
                    </div>

                    <div class="form-group">
                        <label>Calibration Frequency <span class="required-star">*</span></label>
                        <select name="cal_frequency" id="cal_frequency" onchange="updateNextCalibration()" <?= $isGuest ? 'disabled' : 'required' ?>>
                            <option value="">Select frequency</option>
                            <option value="6 months" <?= sel($row['cal_frequency'], '6 months') ?>>6 months</option>
                            <option value="1 year"   <?= sel($row['cal_frequency'], '1 year')   ?>>1 year</option>
                            <option value="2 years"  <?= sel($row['cal_frequency'], '2 years')  ?>>2 years</option>
                            <option value="5 years"  <?= sel($row['cal_frequency'], '5 years')  ?>>5 years</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="cal_type" <?= $isGuest ? 'disabled' : '' ?>>
                            <option value="">— Select —</option>
                            <option value="Internal Calibration" <?= sel($calTypeNormalized, 'Internal Calibration') ?>>Internal Calibration</option>
                            <option value="External Calibration" <?= sel($calTypeNormalized, 'External Calibration') ?>>External Calibration</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label>Environment Type</label>
                        <select name="env_type" <?= $isGuest ? 'disabled' : '' ?>>
                            <option value="">— Select —</option>
                            <option value="Environmental"     <?= sel($row['env_type'] ?? '', 'Environmental')     ?>>Environmental</option>
                            <option value="Non-Environmental" <?= sel($row['env_type'] ?? '', 'Non-Environmental') ?>>Non-Environmental</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="present_status" id="present_status" <?= $isGuest ? 'disabled' : '' ?>>
                            <option value="Good"               <?= sel($row['present_status'], 'Good')               ?>>Good</option>
                            <option value="Not Good"           <?= sel($row['present_status'], 'Not Good')           ?>>Not Good</option>
                            <option value="Disposed"           <?= sel($row['present_status'], 'Disposed')           ?>>Disposed</option>
                            <option value="For Disposal"       <?= sel($row['present_status'], 'For Disposal')       ?>>For Disposal</option>
                            <option value="For Repair"         <?= sel($row['present_status'], 'For Repair')         ?>>For Repair</option>
                            <option value="For Replacement"    <?= sel($row['present_status'], 'For Replacement')    ?>>For Replacement</option>
                            <option value="Missing"            <?= sel($row['present_status'], 'Missing')            ?>>Missing</option>
                            <option value="Not In Use"         <?= sel($row['present_status'], 'Not In Use')         ?>>Not In Use</option>
                            <option value="Not Yet Calibrated" <?= sel($row['present_status'], 'Not Yet Calibrated') ?>>Not Yet Calibrated</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group span-3">
                        ...
                        <textarea name="remarks" rows="3"
                                  <?= $isGuest ? 'readonly' : '' ?>
                                  placeholder="Optional notes or remarks…"><?= htmlspecialchars($row['remarks'] ?? '') ?></textarea>
                    </div>
...
<div id="deleteOverlay" class="modal-overlay">
    <div class="confirm-modal-box">
        <div class="confirm-modal-icon">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 011-1h4a1 1 0 011 1v2"/></svg>
        </div>
        <div class="confirm-modal-body">
            <h3>Delete this record?</h3>
            <p><strong><?= htmlspecialchars($row['machine_code']) ?></strong> — <?= htmlspecialchars($row['description']) ?><br>This action cannot be undone.</p>
        </div>
        <div class="confirm-modal-footer">
            <a href="delete_calibration.php?id=<?= $id ?>&page=<?= $returnPage ?><?= $fromDashboard ? '&from=dashboard' : '' ?>" class="btn btn-danger">
...
function syncModelMaker() {
    const model = document.getElementById('model_input').value.trim();
    const maker = document.getElementById('maker_input').value.trim();
    document.getElementById('model_maker_display').value = [model, maker].filter(Boolean).join('/');
}

function updateNextCalibration() {
    const freq         = document.getElementById('cal_frequency').value;
    const calDateInput = document.getElementById('calibration_date');
    const nextCalInput = document.getElementById('next_calibration');
    if (!calDateInput.value || !freq) { nextCalInput.value = ''; return; }
    let calDate = new Date(calDateInput.value);
    let amount  = parseInt(freq);
    let unit    = freq.split(' ')[1];
    if (unit.includes('month'))     calDate.setMonth(calDate.getMonth() + amount);
    else if (unit.includes('year')) calDate.setFullYear(calDate.getFullYear() + amount);
...
/* ── Exclusion/Inclusion direction logic ──
   Neutral statuses: Good, Not Yet Calibrated
   neutral → non-neutral  = 'excluded'
   non-neutral → neutral  = 'included'
   neutral ↔ neutral      = null (no modal)
   non-neutral ↔ non-neutral = null (no modal)
*/
function getChangeDirection(originalStatus, newStatus) {
    const neutralStatuses = ['good', 'not yet calibrated'];
    const wasNeutral   = neutralStatuses.includes(originalStatus.toLowerCase());
    const isNowNeutral = neutralStatuses.includes(newStatus.toLowerCase());
    if (wasNeutral && !isNowNeutral)  return 'excluded';
    if (!wasNeutral && isNowNeutral)  return 'included';
    return null;
}

function openExclusionModal(direction, machineCode, description, newStatus) {
    ...
    banner.innerHTML = isExcluded
        ? `Status is changing from a neutral status (<strong>Good</strong> / <strong>Not Yet Calibrated</strong>) to a non-Good status. This record will be automatically added to the <strong>Exclusion Summary</strong> as <strong>Excluded</strong>. Please fill in the details below.`
        : `Status is changing from a non-Good status back to a neutral status (<strong>Good</strong> / <strong>Not Yet Calibrated</strong>). This record will be added to the <strong>Exclusion Summary</strong> as <strong>Included</strong>. Please fill in the details below.`;
    ...
    document.getElementById('excl_equipment_display').value = machineCode + ' — ' + description;
    ...
    // Store direction on overlay so confirm handler can read it
...
document.getElementById('editCalForm').addEventListener('submit', function(e) {
    ...
    if (direction) {
        e.preventDefault();
        const machineCode = document.querySelector('[name="machine_code"]').value;
        const description = document.querySelector('[name="description"]').value;
        openExclusionModal(direction, machineCode, description, newStatus);
    }
});
...
document.getElementById('exclConfirmBtn').addEventListener('click', async () => {
    ...
    // Read the direction that was stored when the modal opened
    const direction = document.getElementById('exclusionOverlay').dataset.direction || 'excluded';

    const formFd = new FormData(document.getElementById('editCalForm'));
    const exclFd = new FormData();
    exclFd.append('description',    document.querySelector('[name="description"]').value);
    exclFd.append('control_number', document.querySelector('[name="machine_code"]').value);
    exclFd.append('model_maker',    document.querySelector('[name="model_maker"]').value);
    exclFd.append('serial_number',  document.querySelector('[name="serial_number"]').value);
    ...
    exclFd.append('present_status', document.getElementById('present_status').value);
    exclFd.append('exclusion_type', direction); // ← 'excluded' or 'included', correctly derived
    exclFd.append('remarks',        remarks);
    exclFd.append('recorded_by',    recordedBy);

    try {
        const [, exclResp] = await Promise.all([
            fetch('edit_calibration.php?id=' + RECORD_ID + '&page=' + RETURN_PAGE, { method: 'POST', body: formFd }),
...
document.addEventListener('DOMContentLoaded', () => { updateNextCalibration(); syncModelMaker(); });
</script>
