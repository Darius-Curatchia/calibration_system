    ...
    try {
        $update = $pdo->prepare("
            UPDATE standard_samples SET
                ...
                model=:model, maker=:maker, model_maker=:model_maker,
                serial_no=:serial_no, location=:location,
                section=:section, manager=:manager, reference=:reference,
                calibration_date=:calibration_date,
                next_calibration_date=:next_calibration_date,
                calibration_frequency=:calibration_frequency,
                calibrator=:calibrator, cal_type=:cal_type, env_type=:env_type,
                present_status=:present_status, remarks=:remarks
            WHERE id=:id
        ");
        $update->execute([
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="maker" id="maker_input"
                               value="<?= htmlspecialchars($row['maker'] ?? '') ?>"
                               <?= $isGuest ? 'readonly' : '' ?>
                               oninput="syncModelMaker()">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="model_maker" id="model_maker_display"
                               ...
                               class="auto-value-green" readonly>
                    </div>
                    ...
                    <div class="form-group">
                        <label>Section</label>
                        <input type="text" name="section" value="<?= htmlspecialchars($row['section'] ?? '') ?>" <?= $isGuest ? 'readonly' : '' ?>>
                    </div>

                    <div class="form-group">
                        <label>Manager</label>
                        <input type="text" name="manager" value="<?= htmlspecialchars($row['manager'] ?? '') ?>" <?= $isGuest ? 'readonly' : '' ?>>
                    </div>

                    <div class="form-group">
                        <label>Reference</label>
                        <input type="text" name="reference" class="mono-input" value="<?= htmlspecialchars($row['reference'] ?? '') ?>" <?= $isGuest ? 'readonly' : '' ?>>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <?php
                            $calTypeVal = $row['cal_type'] ?? '';
                            if ($calTypeVal === 'Internal') $calTypeVal = 'Internal Calibration';
                            if ($calTypeVal === 'External') $calTypeVal = 'External Calibration';
                        ?>
                        <select name="cal_type" <?= $isGuest ? 'disabled' : '' ?>>
                            <option value="">— Select —</option>
                            <option value="Internal Calibration" <?= $calTypeVal === 'Internal Calibration' ? 'selected' : '' ?>>Internal Calibration</option>
                            <option value="External Calibration" <?= $calTypeVal === 'External Calibration' ? 'selected' : '' ?>>External Calibration</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="env_type" <?= $isGuest ? 'disabled' : '' ?>>
                            <option value="">— Select —</option>
                            <option value="Environmental"     <?= ($row['env_type'] ?? '') === 'Environmental'     ? 'selected' : '' ?>>Environmental</option>
                            <option value="Non-Environmental" <?= ($row['env_type'] ?? '') === 'Non-Environmental' ? 'selected' : '' ?>>Non-Environmental</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group" style="grid-column:1/-1;">
                        <label>Remarks</label>
                        <textarea name="remarks" rows="3" <?= $isGuest ? 'readonly' : '' ?>
                                  placeholder="Optional notes or remarks…"
                                  style="padding:8px 12px;resize:vertical;line-height:1.55;min-height:72px;border-radius:var(--r-sm);border:1px solid var(--border-mid);font-size:12.5px;font-family:'Plus Jakarta Sans',sans-serif;color:var(--text);background:var(--bg-raised);width:100%;"><?= htmlspecialchars($row['remarks'] ?? '') ?></textarea>
                    </div>
