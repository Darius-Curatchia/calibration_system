window.addEventListener('DOMContentLoaded', () => {
    ...
    presetChips.forEach(chip => {
        chip.addEventListener('click', () => {
            if (chip.classList.contains('active')) { chip.classList.remove('active'); dateFrom.value = ''; dateTo.value = ''; onRangeChanged(); filterRows(true); return; }
            presetChips.forEach(c => c.classList.remove('active'));
    ...
    function filterRows(resetPage = false) {
        ...
        filteredRows = Array.from(tableBody.querySelectorAll('tr.data-row')).filter(row => {
            ...
            const raw    = row.dataset.nextInspectionRaw || '';
            let dateMatch = true;
            if (fromVal || toVal) {
    ...
    function buildFilterNote() {
        ...
        if (dateFrom && (dateFrom.value || dateTo.value)) parts.push(`Next Inspection: ${dateFrom.value || '…'} → ${dateTo.value || '…'}`);
