...
// Ensure all extra columns exist in standard_samples (safe to run every time — errors mean already exists)
foreach ([
    "ALTER TABLE standard_samples ADD COLUMN cal_type    TEXT NOT NULL DEFAULT ''",
    "ALTER TABLE standard_samples ADD COLUMN item_no     TEXT NOT NULL DEFAULT ''",
    "ALTER TABLE standard_samples ADD COLUMN model       TEXT NOT NULL DEFAULT ''",
    "ALTER TABLE standard_samples ADD COLUMN maker       TEXT NOT NULL DEFAULT ''",
    "ALTER TABLE standard_samples ADD COLUMN section     TEXT NOT NULL DEFAULT ''",
    "ALTER TABLE standard_samples ADD COLUMN manager     TEXT NOT NULL DEFAULT ''",
    "ALTER TABLE standard_samples ADD COLUMN reference   TEXT NOT NULL DEFAULT ''",
    "ALTER TABLE standard_samples ADD COLUMN env_type    TEXT NOT NULL DEFAULT ''",
    "ALTER TABLE standard_samples ADD COLUMN remarks     TEXT",
] as $sql) {
...
$stmt = $pdo->query("SELECT * FROM standard_samples ORDER BY equipment_code ASC");
$allRows = $stmt->fetchAll(PDO::FETCH_ASSOC);

$calibratorStmt = $pdo->query("SELECT DISTINCT calibrator FROM standard_samples WHERE calibrator IS NOT NULL AND calibrator != '' ORDER BY calibrator ASC");
$calibrators = $calibratorStmt->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Standard Samples — Calibration Management</title>
...
.calibration-table{width:100%;border-collapse:collapse;table-layout:auto;font-size:12.5px;min-width:900px;}
...
.calibration-table td{padding:9px 12px;border-bottom:1px solid var(--border);border-right:1px solid rgba(5,48,79,0.05);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;vertical-align:middle;color:var(--text);}
.calibration-table td:last-child{border-right:none;}
.calibration-table tr:last-child td{border-bottom:none;}
.serial-no-cell{max-width:130px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
...
/* ── NEW: Not Yet Calibrated ── */
.status-not-yet-calibrated{background:rgba(139,92,246,0.10);color:#5b21b6;}
...
.calibration-table tbody tr.row-overdue:hover,
.calibration-table tbody tr.row-this-month:hover,
.calibration-table tbody tr.row-next-month:hover,
.calibration-table tbody tr.row-external:hover{background:rgba(26,144,217,0.06)!important;box-shadow:inset 3px 0 0 var(--accent);}
...
.no-results-row td{text-align:center;padding:60px 20px!important;color:var(--text-3);font-size:13px;border-right:none!important;width:100%!important;cursor:default;}
...
.detail-field-value.remarks-value{white-space:pre-wrap;word-break:break-word;font-size:12.5px;color:var(--text-3);font-style:italic;line-height:1.55;}
...
<div class="main-content">
    ...
    <div class="card">
        <div class="card-header">
            <h2>List of Standard Samples</h2>
        ...
        <div class="card-body">
            <div class="top-controls">
                <div class="controls-left">
                    <?php if (!$isGuest): ?>
                    <button class="btn btn-primary" onclick="window.location.href='add_standard_sample.php?page=<?= $currentPageFromQuery ?>'">
                ...
                <div class="controls-right">
                    ...
                    <select class="filter-select" id="calibratorFilter">
                        ...
                        <?php foreach ($calibrators as $row): ?>
                        <option value="<?= htmlspecialchars($row['calibrator']) ?>"><?= htmlspecialchars($row['calibrator']) ?></option>
                        <?php endforeach; ?>
                    </select>
                    <select class="filter-select" id="calTypeFilter">
                        <option value="">All Cal Types</option>
                        <option value="Internal Calibration">Internal</option>
                        <option value="External Calibration">External</option>
                    </select>
                    <button class="btn btn-muted" id="toggleDateRangeBtn" title="Filter by Next Calibration date range">
            ...
            <form method="POST" action="delete_multiple_standard_samples.php" id="samplesForm">
                <input type="hidden" name="page" id="currentPageInput">
                <div id="deleteHiddenInputs"></div>
                <div class="table-container">
                    <table class="calibration-table" id="samplesTable" data-mode="<?= $isGuest ? 'guest' : 'full' ?>">
                        <thead>
                            <tr>
                        ...
                        while ($r = $stmt->fetch(PDO::FETCH_ASSOC)):
                            ...
                            switch($r['present_status']) {
                                ...
                                case 'Not Yet Calibrated': $statusClass = 'status-not-yet-calibrated'; break;
                            }
                            $e = fn($v) => htmlspecialchars((string)($v ?? ''), ENT_QUOTES|ENT_SUBSTITUTE, 'UTF-8');
                            $ctRaw = $r['cal_type'] ?? '';
                            if ($ctRaw === 'Internal') $ctRaw = 'Internal Calibration';
                            if ($ctRaw === 'External') $ctRaw = 'External Calibration';
                            $da  = ' data-id="'        .$r['id'].'"';
                            $da .= ' data-description="'    .$e($r['description']).'"';
                            $da .= ' data-equipment-code="' .$e($r['equipment_code']).'"';
                            $da .= ' data-model-maker="'    .$e($r['model_maker']).'"';
                            $da .= ' data-model="'          .$e($r['model'] ?? '').'"';
                            $da .= ' data-maker="'          .$e($r['maker'] ?? '').'"';
                            $da .= ' data-serial-no="'      .$e($r['serial_no']).'"';
                            $da .= ' data-location="'       .$e($r['location']).'"';
                            $da .= ' data-section="'        .$e($r['section'] ?? '').'"';
                            $da .= ' data-manager="'        .$e($r['manager'] ?? '').'"';
                            $da .= ' data-reference="'      .$e($r['reference'] ?? '').'"';
                            $da .= ' data-cal-date="'       .formatDateExcel($r['calibration_date']).'"';
                            $da .= ' data-cal-date-raw="'   .$e($r['calibration_date'] ?? '').'"';
                            $da .= ' data-next-cal="'       .formatDateExcel($r['next_calibration_date']).'"';
                            $da .= ' data-next-cal-raw="'   .$e($r['next_calibration_date'] ?? '').'"';
                            $da .= ' data-frequency="'      .formatFrequency($r['calibration_frequency']).'"';
                            $da .= ' data-calibrator="'     .$e($r['calibrator']).'"';
                            $da .= ' data-cal-type="'       .$e($ctRaw).'"';
                            $da .= ' data-env-type="'       .$e($r['env_type'] ?? '').'"';
                            $da .= ' data-remarks="'        .$e($r['remarks'] ?? '').'"';
                            $da .= ' data-status="'         .$e($r['present_status']).'"';
                        ...
                        <tr class="data-row" <?= $da ?>>
                            ...
                            <td class="serial-no-cell" title="<?= htmlspecialchars($r['serial_no']) ?>"><?= htmlspecialchars($r['serial_no']) ?></td>
                            <td><?= htmlspecialchars($r['location']) ?></td>
                            <td class="date-cell"><?= formatDateExcel($r['calibration_date']) ?></td>
                            <td class="date-cell next-cal-cell" data-raw="<?= htmlspecialchars($r['next_calibration_date'] ?? '', ENT_QUOTES) ?>"><?= formatDateExcel($r['next_calibration_date']) ?></td>
                            <td style="font-size:12px;"><?= formatFrequency($r['calibration_frequency']) ?></td>
...
<div id="detailModal" class="modal-overlay">
    <div class="detail-modal-box">
        <div class="detail-modal-header">
            <div class="detail-modal-header-text"><h3 id="modal-description">—</h3><p id="modal-equipment-code">—</p></div>
        ...
        <div class="detail-modal-body">
            ...
            <div class="detail-field"><div class="detail-field-label">Serial Number</div><div class="detail-field-value mono" id="modal-serial-no">—</div></div>
...
<script src="assets/js/standard_samples.js" defer></script>
</body>
</html>